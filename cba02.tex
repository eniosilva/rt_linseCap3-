...
\begin{abstract}
 Este trabalho apresenta uma estratégia de seleção de atributos característicos do sinal de fala objetivando o aprimoramento da classificação em grupos fonéticos abrangentes (\textit{broad group phonetic} - BGP) com ênfase na discriminação dos fonemas pertinentes ao grupo fonético fricativo. Os atributos analisados são investigados através de métodos baseados em algoritmos de aprendizagem de máquinas (\textit{machine learning}) e análise da informação mútua (\textit{mutual information} - MI), seguido do descarte de componentes de menor importância. Assim, a estratégia proposta consiste em identificar os atributos que facilitam a distinção entre diferentes BGP. Os resultados das simulações permitem inferir acerca da seleção dos melhores atributos a serem usados na composição de classificadores fonéticos.
...
\end{abstract}
\keywords{Reconhecimento de padrões, classificação fonética, seleção de atributos.}
]
\selectlanguage{brazil}
...
A atividade de classificação de padrões é um processo inerente do ser humano. Para distinguir um objeto de outro, normalmente, o ser humano armazena parâmetros marcantes, aqui chamados de atributos característicos, de um determinado conjunto. Assim, os métodos de seleção tentam identificar e reter apenas os atributos que mais contribuem para a execução de uma determinada tarefa \cite{marinho2004}.
...
Em \cite{Klautau2003} a seleção de atributos foi aplicada à sistemas de reconhecimento automático de fala e tem como base o algoritmo \textit{AdaBoost}.  Em \cite{Hamooni2014} a distância DTW (\textit{Dynamic Time Warping}) e os coeficientes cepstrais da escala mel (\textit{Mel-frequency cepstrum coefficients} - MFCC) são utilizados como atributos para uma classificação fonética hierárquica usando os domínios do tempo e frequência. Já em \cite{Alex2010}, uma série de atributos, tais como \textit{spectral peak locations}, \textit{spectral rolloff}, \textit{spectral centroid}, dentre outros, são considerados para a classificação de subconjuntos fonéticos. 

Em \cite{Patricia2007} um classificador fonético é proposto com base na especialização de classificadores em BPG e na seleção de atributos de tempo e frequência. Tais classificadores enfatizam a capacidade de discriminação de quatro diferentes BGP  (vogais, nasais, fricativas e oclusivas) para, posteriormente, discriminar os fonemas pertinentes a cada BGP correspondente. Dessa maneira, são obtidas melhores taxas de desempenho quando comparado com a classificação isolada dos fonemas. Também em \cite{Patricia2007}, a informção mútua (\textit{mutual information - MI}) é usada como função objetivo para seleção dos atributos ótimos usados como entrada dos classificadores. Todavia, mesmo com a seleção de atributos ótimos, a estratégia proposta necessita de uma vasta quantidade de atributos para obter um desempenho satisfatório.
...
Nos referidos trabalhos, os sinais considerados como referência são sinais de fala de banda larga, estes possuem geralmente frequência de amostragem superior à 16000 Hz e contêm componentes de frequências superiores à 3400 Hz. Em contraste, os algoritmos discutidos em \cite{Morales2009}, \cite{He2011} e \cite{Lee2012} utilizam como referência, sinais de fala de banda limitada aplicados à rede de telefonia pública (\textit{public switched telephone network} - PSTN). 

Entretanto, apesar da crescente evolução dos algoritmos de classificação fonética, ainda não se dispõe de qualquer procedimento consolidado apresentando desempenho satisfatório para sinais de banda limitada. Desse modo as estratégias propostas apresentam desempenhos severamente degradados sempre que a classe fonética não é precisamente obtida. 
...
Assim, objetivando a seleção de atributos para a classificação fonética de sinais de fala de banda limitada, neste trabalho a estratégia proposta tem como base os algoritmos de árvore de decisão (J.48), redes neurais de múltiplas camadas (MLP) e KNN, seguido da análise da informação mútua para o descarte dos atributos de menor relevância \cite{Webb2002}. Dessa forma, a estratégia proposta consiste em identificar os atributos que facilitam a distinção entre grupos fonéticos diferentes. Os resultados das simulações permitem inferir acerca da seleção dos melhores atributos a serem usados na composição de um classificador fonético e comprovam a eficácia da estratégia proposta.
...
A tarefa de classificação é parte do processo de reconhecimento de padrões. Independente do sistema que está sendo implementado (reconhecimento de fala, impressões digitais ou mesmo classificação de frutas em um processo industrial de fabricação de sucos), a tarefa de classificação é considerada um ponto chave do reconhecimento de padrões \cite{Maimon2010}.% A tarefa de classificar um objeto em um pré-especificado conjunto de categorias ou classes é uma característica da inteligência humana, que é objeto de estudos da inteligência artificial.
...
\includegraphics[scale=0.2]{figures/tree.eps}
\caption{\label{tree}Árvore de decisão fonética.}
\end{figure}
A Fig. \ref{tree} apresenta diferentes níveis de clusterização, detalhes da classificação proposta é apresentada na Tabela \ref{TebelaClasse}. Após o processo de clusterização as classificações fonéticas tornam-se mais discriminativas e, consequentemente, mais apropriada para a etapa de treinamento.
...
\begin{table}[htb]
  ...
  \begin{center}
    \begin{tabular}{|c|l|}\hline
      ...
      $Cl_1$ (ex.$/z/$)& Fricativas vozeadas alveolares\\\hline
      $Cl_2$ (ex.$/v/$)& Fricativas vozeadas labiodentais\\\hline
      $Cl_3$ (ex.$/j/$)& Fricativas vozeadas palatais\\\hline
      $Cl_4$ (ex.$/a/$)& Demais fonemas vozeados\\\hline
      $Cl_5$ (ex.$/s/$)& Fricativas não-vozeadas alveolares\\\hline
      $Cl_6$ (ex.$/f/$)& Fricativas não-vozeadas labiodentais\\\hline
      $Cl_7$ (ex.$/x/$)& Fricativas não-vozeadas palatais\\\hline
      $Cl_8$ (ex.$/b/$)& Demais fonemas não-vozeados\\\hline
    \end{tabular}
  \end{center}
\end{table}
...
Assim como em \cite{Jax2004}, os seguintes atributos vetoriais e escalares são investigados:
\begin{itemize}
