...
\scalefont{0.7}
  ...
  Classes & BGP & Vozeamento & Ponto de Articulação\\\hline\hline
...
O sistema proposto de classificação hierárquica, primeiro analisa a qual BGP cada \textit{frame} pertence. Caso o \textit{frame} analisado pertença ao grupo fonético fricativo, um novo classificador é utilizado para a detecção de classes vozeadas ou não-vozeadas. Finalmente, estimado o vozeamento do \textit{frame}, um terceiro classificador é aplicado para identificação do tipo de fonema fricativo. A Fig. TAL ilustra o processo de classificação hieráquica proposto.

FIGURA TAL

Para cada nível hierárquico de classificação um subconjunto dos atributos de entrada, $\mathbf{x} = (x_1,\ldots,x_L)$, é utilizado. Esses subconjuntos são previamente investigados conforme as mátricas de seleção de atributos descritas na Seção TAL e constituem a etapa de \textit{front-end} dos classificadores.


\section{Resultados e Análise de Desempenho}\label{resultados}
Objetivando a comparação da eficácia da seleção de atributos para os diferentes níveis hierárquicos, os conjuntos de atributos selecionados pelas técnicas MI e CSF foram testados nos classificadores listados na Seção TAL. A fig. TAL apresenta a evolução da precisão de classificação em relação ao número de atributos selecionados. 
