...
\begin{abstract}
 Este trabalho apresenta uma estratégia de seleção de atributos característicos do sinal de fala objetivando aprimorar a classificação em grupos fonéticos. Os atributos analisados são investigados através de métodos baseados em algoritmos de \textit{machine learning}, técnicas de \textit{principal component analysis} (PCA) e \textit{mutual information} (MI), seguido do descarte de componentes de menor importância. Assim, a estratégia proposta consiste em identificar os atributos que facilitam a distinção entre grupos fonéticos diferentes. Os resultados das simulações permitem inferir acerca da seleção dos melhores atributos a serem usados na composição de classificadores fonéticos.
