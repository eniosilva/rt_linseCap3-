...
A Tabela \ref{desempenho} apresenta os classificadores de melhor desempenho, bem como a quantidade de atributos de cada nível hierárquico. Na Tabela \ref{desempenho}, também observa-se o desempenho geral do \textit{framework} de classificação proposto em contraste com o sistema padrão de classficação direta. Nesse contexto, as taxas de erro de classificação dos subgrupos fricativos, $E_{rro}(C_{l4vf})$, são calculados segundo a Equação (\ref{PropClass}).
...
O \textit{framework} de classificação, composto pelos atributos e classificadores descritos na Tabela \ref{desempenho}, é examinado através da comparação de suas matrizes de confusão fonética, apresentadas na Fig. \ref{evolucao}, com a matriz obtida pelo sistema padrão de classficação direta apresentada na Fig. \ref{matrix}. Nesse contexto, nota-se uma menor confusão na matriz resultante do \textit{framework} aqui proposto, confirmando, assim, sua eficácia. A Fig. \ref{desempInd} apresenta os desempenhos individuais do sistema padrão e do \textit{framework} proposto para cada classe fonética.
...
\caption{\label{desempInd}Desempenho final dos classificadores.}
\end{figure}
